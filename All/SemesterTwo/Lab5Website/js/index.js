...
v 4.0.0 - 3/7/2024 - Modified from to use regular expressions and reformatted to not have text inside answer field
***
******************************************************************** */
$(document).ready( () => {
    let userEntry = "";//Records user response
    let userEntryParsed = $("#ant_guess").val();//Records user response but parsed, this is just for math
    let systemResponse = "";
    //creates validation
    const guessValidate = /^(\d{1,3})(,(\d{1,3})){1,}$/;//confirms number
    ...
    const runAntGuessCheck = (userEntry) => {
        ...
        //Checks validation
        else if(!guessValidate.test(userEntry)){
            systemResponse = ("Answer not in the right format!");
        }
        //Answer right
        else if (userEntry == "20,000,000,000,000,000"){

...
v 4.0.0 - 3/7/2024 - repositioned code to span reset so * shows up on start, added date picker, focus disabled
                     as it felt wrong to have the page not display the top of the page when loading the page
***
******************************************************************** */
$(document).ready( () => {
    //makes datepicker and gets current date
    $("#datepicker").datepicker();
    const currentDay = $("#datepicker").datepicker("getDate");
    //hides date picker
    $("#date_holder").slideUp(1);
    //hides/shows date picker
    $("#newsletter_date").click(() => {
        $("#date_holder").toggleClass("hidden");
        //toggles to shown
        if($("#date_holder").attr("class") == "hidden"){
            $("#date_holder").fadeOut(400);
            $("#date_holder").slideUp(1000);
            $("#date_holder").toggle("clip");
            $("#newsletter_date").val("Close");
        }
        //toggles to closed
        else{
            $("#date_holder").fadeIn(400);
            $("#date_holder").slideDown(1000);
            $("#date_holder").toggle("clip");
            $("#newsletter_date").val("Open");
        }
    });
    //submit validation
    $("#submit_form").click(() => {
        //clears previous data
        $("#questions div span").text("");
        ...
        //gets date picked and formats system response
        let pickedDate = $("#datepicker").datepicker("getDate");
        let changedDate = pickedDate.toString(pickedDate);
        let formattedDate = changedDate.substring(0,15);
        if(currentDay > pickedDate){
            $("#date_picked").text("You will receive the newsletter on " + formattedDate + ", We'll get the time machine!");
        }
        else{
            $("#date_picked").text("You will receive the newsletter on " + formattedDate);
        }
    });
    //reset form
    $("#reset_form").click( () => {
